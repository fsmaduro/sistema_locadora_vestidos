object inserir_manequim: Tinserir_manequim
  Left = 185
  Top = 175
  BorderIcons = []
  BorderStyle = bsSingle
  Caption = 'AMVSystem 1.0 | DEFINIR MANEQUIM'
  ClientHeight = 359
  ClientWidth = 421
  Color = 12910591
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnClose = FormClose
  OnCreate = FormCreate
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object btn_ok: TSpeedButton
    Left = 104
    Top = 295
    Width = 76
    Height = 57
    Caption = '&OK'
    Flat = True
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFF}
    Layout = blGlyphTop
    ParentShowHint = False
    ShowHint = True
    Spacing = 3
    OnClick = btn_okClick
  end
  object btn_sair: TSpeedButton
    Left = 240
    Top = 295
    Width = 76
    Height = 57
    Hint = 'Sair'
    Caption = '&Sair'
    Flat = True
    Glyph.Data = {
      
      0000}
    Layout = blGlyphTop
    ParentShowHint = False
    ShowHint = True
    OnClick = btn_sairClick
  end
  object grdManequim: TwwDBGrid
    Left = 0
    Top = 8
    Width = 420
    Height = 281
    ControlType.Strings = (
      'valor;CustomEdit;cmbManequim1;F')
    Selected.Strings = (
      'DESCRICAO'#9'50'#9'DESCRICAO'#9'F'
      'VALOR'#9'15'#9'VALOR'#9'F')
    IniAttributes.Delimiter = ';;'
    TitleColor = 16312551
    FixedCols = 1
    ShowHorzScrollBar = True
    Color = 16312551
    Ctl3D = True
    DataSource = dtsManequim
    Options = [dgEditing, dgTabs, dgConfirmDelete, dgCancelOnExit, dgWordWrap]
    ParentCtl3D = False
    TabOrder = 0
    TitleAlignment = taLeftJustify
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    TitleLines = 1
    TitleButtons = False
    UseTFields = False
    FooterColor = 16312551
    FooterCellColor = 16312551
    PadColumnStyle = pcsPlain
  end
  object grdManequimProduto: TwwDBGrid
    Left = 0
    Top = 0
    Width = 420
    Height = 289
    ControlType.Strings = (
      'valor;CustomEdit;cmbManequimProduto1;F'
      'valorfinal;CustomEdit;cmbManequimProduto2;F')
    Selected.Strings = (
      'DESCRICAO'#9'39'#9'DESCRICAO'#9'F'
      'VALOR'#9'15'#9'VALOR'#9'F'
      'valorfinal'#9'10'#9'valorfinal'#9'F')
    IniAttributes.Delimiter = ';;'
    TitleColor = 9699327
    FixedCols = 1
    ShowHorzScrollBar = True
    Color = 9699327
    Ctl3D = True
    DataSource = dtsManequimProduto
    Options = [dgEditing, dgTabs, dgConfirmDelete, dgCancelOnExit, dgWordWrap]
    ParentCtl3D = False
    TabOrder = 1
    TitleAlignment = taLeftJustify
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    TitleLines = 1
    TitleButtons = False
    UseTFields = False
    FooterColor = 9699327
    FooterCellColor = 9699327
    PadColumnStyle = pcsPlain
  end
  object cmbManequim1: TwwDBLookupCombo
    Left = 8
    Top = 122
    Width = 109
    Height = 21
    DropDownAlignment = taLeftJustify
    Selected.Strings = (
      'VALORINICIAL'#9'6'#9'Valor Inicial'#9'F'
      'VALORFINAL'#9'6'#9'Valor Final'#9'F'
      'DESCRICAO'#9'15'#9'Descri'#231#227'o'#9'F'#9)
    DataField = 'valor'
    DataSource = dtsManequim
    LookupTable = qryValorPadrao
    LookupField = 'VALORINICIAL'
    Options = [loSearchOnBackspace]
    DropDownCount = 16
    TabOrder = 2
    AutoDropDown = True
    ShowButton = True
    PreciseEditRegion = False
    AllowClearKey = False
    ShowMatchText = True
    OnKeyPress = cmbManequim1KeyPress
  end
  object cmbManequimProduto1: TwwDBLookupCombo
    Left = 8
    Top = 177
    Width = 109
    Height = 21
    DropDownAlignment = taLeftJustify
    Selected.Strings = (
      'VALORINICIAL'#9'6'#9'Valor'#9'F'#9)
    DataField = 'valor'
    DataSource = dtsManequimProduto
    LookupTable = qryValorPadrao
    LookupField = 'VALORINICIAL'
    Options = [loSearchOnBackspace]
    DropDownCount = 16
    TabOrder = 3
    AutoDropDown = True
    ShowButton = True
    PreciseEditRegion = False
    AllowClearKey = False
  end
  object cmbManequimProduto2: TwwDBLookupCombo
    Left = 8
    Top = 204
    Width = 109
    Height = 21
    DropDownAlignment = taLeftJustify
    Selected.Strings = (
      'VALORINICIAL'#9'6'#9'Valor'#9'F'#9)
    DataField = 'valorfinal'
    DataSource = dtsManequimProduto
    LookupTable = qryValorPadrao
    LookupField = 'VALORINICIAL'
    Options = [loSearchOnBackspace]
    DropDownCount = 16
    TabOrder = 4
    AutoDropDown = True
    ShowButton = True
    PreciseEditRegion = False
    AllowClearKey = False
  end
  object qryGeral: TIBQuery
    Database = Dados.IBDatabase
    Transaction = Dados.IBTransaction
    Left = 255
    Top = 74
  end
  object memManequim: TRxMemoryData
    FieldDefs = <
      item
        Name = 'codigo'
        DataType = ftInteger
      end
      item
        Name = 'descricao'
        DataType = ftString
        Size = 100
      end
      item
        Name = 'valor'
        DataType = ftFloat
      end>
    BeforeInsert = memManequimBeforeInsert
    BeforeDelete = memManequimBeforeDelete
    AfterScroll = memManequimAfterScroll
    Left = 250
    Top = 146
    object memManequimcodigo: TIntegerField
      FieldName = 'codigo'
    end
    object memManequimdescricao: TStringField
      FieldName = 'descricao'
      Size = 100
    end
    object memManequimvalor: TFloatField
      FieldName = 'valor'
    end
  end
  object dtsManequim: TDataSource
    DataSet = memManequim
    Left = 256
    Top = 176
  end
  object memManequimProduto: TRxMemoryData
    FieldDefs = <
      item
        Name = 'codigo'
        DataType = ftInteger
      end
      item
        Name = 'descricao'
        DataType = ftString
        Size = 100
      end
      item
        Name = 'valor'
        DataType = ftFloat
      end
      item
        Name = 'valorfinal'
        DataType = ftFloat
      end>
    BeforeInsert = memManequimBeforeInsert
    BeforeDelete = memManequimBeforeDelete
    AfterScroll = memManequimProdutoAfterScroll
    Left = 289
    Top = 144
    object memManequimProdutocodigo: TIntegerField
      FieldName = 'codigo'
    end
    object memManequimProdutodescricao: TStringField
      FieldName = 'descricao'
      Size = 100
    end
    object memManequimProdutovalor: TFloatField
      FieldName = 'valor'
    end
    object memManequimProdutovalorfinal: TFloatField
      FieldName = 'valorfinal'
    end
  end
  object dtsManequimProduto: TDataSource
    DataSet = memManequimProduto
    Left = 288
    Top = 176
  end
  object qryValorPadrao: TIBQuery
    Database = Dados.IBDatabase
    Transaction = Dados.IBTransaction
    BeforeOpen = qryValorPadraoBeforeOpen
    SQL.Strings = (
      'SELECT *'
      '  FROM TABTIPOMANEQUIMVALORPADRAO'
      'WHERE 1 = 1'
      ''
      '--AND'
      ''
      ''
      '--')
    Left = 253
    Top = 228
    object qryValorPadraoVALORINICIAL: TIBBCDField
      DisplayLabel = 'Valor Inicial'
      DisplayWidth = 6
      FieldName = 'VALORINICIAL'
      Origin = '"TABTIPOMANEQUIMVALORPADRAO"."VALORINICIAL"'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
      Precision = 18
      Size = 2
    end
    object qryValorPadraoVALORFINAL: TIBBCDField
      DisplayLabel = 'Valor Final'
      DisplayWidth = 6
      FieldName = 'VALORFINAL'
      Origin = '"TABTIPOMANEQUIMVALORPADRAO"."VALORFINAL"'
      Visible = False
      Precision = 18
      Size = 2
    end
    object qryValorPadraoDESCRICAO: TIBStringField
      DisplayLabel = 'Descri'#231#227'o'
      DisplayWidth = 15
      FieldName = 'DESCRICAO'
      Origin = '"TABTIPOMANEQUIMVALORPADRAO"."DESCRICAO"'
      Visible = False
      Size = 200
    end
    object qryValorPadraoCODIGOTIPOMANEQUIM: TIntegerField
      DisplayWidth = 10
      FieldName = 'CODIGOTIPOMANEQUIM'
      Origin = '"TABTIPOMANEQUIMVALORPADRAO"."CODIGOTIPOMANEQUIM"'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
      Visible = False
    end
  end
end
