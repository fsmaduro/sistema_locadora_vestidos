object visualizar_historico: Tvisualizar_historico
  Left = 185
  Top = 175
  BorderIcons = [biSystemMenu, biHelp]
  BorderStyle = bsSingle
  Caption = 'AMVSystem 1.0 | AVALIAR'
  ClientHeight = 377
  ClientWidth = 789
  Color = 12910591
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnCreate = FormCreate
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object grdManequim: TwwDBGrid
    Left = 0
    Top = 0
    Width = 789
    Height = 313
    ControlType.Strings = (
      'valor;CustomEdit;cmbManequim1;F'
      'VALOR;CustomEdit;wwDBLookupCombo1;F')
    Selected.Strings = (
      'CODIGOORIGEM'#9'4'#9'Id'#9'F'
      'DATA'#9'10'#9'Data'#9'F'#9'In'#237'cio'
      'HORA'#9'10'#9'Hora'#9'F'#9'In'#237'cio'
      'DATAFIM'#9'10'#9'Data'#9'F'#9'Fim'
      'HORAFIM'#9'10'#9'Hora'#9'F'#9'Fim'
      'CODIGOSITUACAO'#9'5'#9'C'#243'digo'#9'F'#9'Situa'#231#227'o'
      'NOMESITUACAO'#9'25'#9'Situa'#231#227'o'#9'F'#9'Situa'#231#227'o'
      'USERNAME'#9'15'#9'Usu'#225'rio'#9'F'
      'OBSERVACOES'#9'30'#9'Observa'#231#245'es'#9'F')
    IniAttributes.Delimiter = ';;'
    TitleColor = 9699327
    FixedCols = 1
    ShowHorzScrollBar = True
    Align = alClient
    Color = clWhite
    Ctl3D = True
    DataSource = dtsHistorico
    KeyOptions = [dgAllowInsert]
    Options = [dgEditing, dgTitles, dgTabs, dgConfirmDelete, dgCancelOnExit, dgWordWrap]
    ParentCtl3D = False
    TabOrder = 0
    TitleAlignment = taLeftJustify
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    TitleLines = 2
    TitleButtons = False
    UseTFields = False
    FooterColor = 16312551
    FooterCellColor = 16312551
    PadColumnStyle = pcsPlain
  end
  object Panel1: TPanel
    Left = 0
    Top = 313
    Width = 789
    Height = 64
    Align = alBottom
    ParentBackground = False
    ParentColor = True
    TabOrder = 1
    ExplicitTop = 319
    object btn_sair: TSpeedButton
      Left = 354
      Top = 3
      Width = 76
      Height = 57
      Hint = 'Sair'
      Caption = '&Sair'
      Flat = True
      Glyph.Data = {
        
        0000}
      Layout = blGlyphTop
      ParentShowHint = False
      ShowHint = True
      OnClick = btn_sairClick
    end
    object lblResultado: TLabel
      Left = 95
      Top = 12
      Width = 5
      Height = 13
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clNavy
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
  end
  object dtsHistorico: TDataSource
    DataSet = qryHistorico
    Left = 294
    Top = 177
  end
  object qryHistorico: TIBQuery
    Database = Dados.IBDatabase
    Transaction = Dados.IBTransaction
    BeforeOpen = qryHistoricoBeforeOpen
    SQL.Strings = (
      
        'select H.*, U.USERNAME, COALESCE(S.DESCRICAO,H.descricaosituacao' +
        ') AS NOMESITUACAO, A.DATAFIM, A.HORAFIM, A.OBSERVACOES'
      '  from TABHISTORICOSITUACAO H'
      
        'INNER JOIN TABAVALIACAOPERFILGERAL A ON (H.CODIGOORIGEM = A.CODI' +
        'GOORIGEM)'
      'LEFT JOIN TABSITUACAOPRODUTO S ON (H.codigosituacao = S.CODIGO)'
      'LEFT JOIN TABUSUARIOS U ON (A.CODIGOUSUARIO = U.CODIGO)'
      ''
      'WHERE A.CODIGOREGISTRO = :CODIGOREGISTRO'
      ''
      'order by data, hora')
    Left = 292
    Top = 125
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'CODIGOREGISTRO'
        ParamType = ptUnknown
      end>
    object qryHistoricoCODIGOORIGEM: TIntegerField
      FieldName = 'CODIGOORIGEM'
      Origin = '"TABHISTORICOSITUACAO"."CODIGOORIGEM"'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object qryHistoricoID: TIntegerField
      FieldName = 'ID'
      Origin = '"TABHISTORICOSITUACAO"."ID"'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object qryHistoricoTIPO: TIntegerField
      FieldName = 'TIPO'
      Origin = '"TABHISTORICOSITUACAO"."TIPO"'
    end
    object qryHistoricoDATA: TDateField
      FieldName = 'DATA'
      Origin = '"TABHISTORICOSITUACAO"."DATA"'
    end
    object qryHistoricoHORA: TTimeField
      FieldName = 'HORA'
      Origin = '"TABHISTORICOSITUACAO"."HORA"'
    end
    object qryHistoricoCODIGOSITUACAO: TIntegerField
      FieldName = 'CODIGOSITUACAO'
      Origin = '"TABHISTORICOSITUACAO"."CODIGOSITUACAO"'
    end
    object qryHistoricoDESCRICAOSITUACAO: TIBStringField
      FieldName = 'DESCRICAOSITUACAO'
      Origin = '"TABHISTORICOSITUACAO"."DESCRICAOSITUACAO"'
      Size = 100
    end
    object qryHistoricoUSERNAME: TIBStringField
      FieldName = 'USERNAME'
      Origin = '"TABUSUARIOS"."USERNAME"'
    end
    object qryHistoricoNOMESITUACAO: TIBStringField
      FieldName = 'NOMESITUACAO'
      ProviderFlags = []
      Size = 200
    end
    object qryHistoricoDATAFIM: TDateField
      FieldName = 'DATAFIM'
      Origin = '"TABAVALIACAOPERFILGERAL"."DATAFIM"'
    end
    object qryHistoricoHORAFIM: TTimeField
      FieldName = 'HORAFIM'
      Origin = '"TABAVALIACAOPERFILGERAL"."HORAFIM"'
    end
    object qryHistoricoOBSERVACOES: TIBStringField
      FieldName = 'OBSERVACOES'
      Origin = '"TABAVALIACAOPERFILGERAL"."OBSERVACOES"'
      Size = 100
    end
  end
  object qryUsuario: TIBQuery
    Database = Dados.IBDatabase
    Transaction = Dados.IBTransaction
    SQL.Strings = (
      'select CODIGO, USERNAME from TABUSUARIOS')
    Left = 641
    Top = 162
    object qryUsuarioCODIGO: TIntegerField
      FieldName = 'CODIGO'
      Origin = '"TABUSUARIOS"."CODIGO"'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object qryUsuarioUSERNAME: TIBStringField
      FieldName = 'USERNAME'
      Origin = '"TABUSUARIOS"."USERNAME"'
    end
  end
  object dtsUsuario: TDataSource
    DataSet = qryUsuario
    Left = 609
    Top = 162
  end
end
