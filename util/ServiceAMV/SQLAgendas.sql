...
SELECT 
   ...
   'Produto: '||Coalesce(Pr.codigo,'')||' - '||Coalesce(pr.descricao,'')||ascii_char(13)||
   ...
   'DESCRIÇÃO: '||ascii_char(13)||
   A.descricao||ascii_char(13)||ascii_char(13)||

   'PREFERENCIAS: '||ascii_char(13)||
   COALESCE(
   (select first 1 list(xpto, ascii_char(13))
   from (Select Trim(Case when px.tipo = 1 then 'Peça'
                ...
                else 'OUTRAS' END||ascii_char(13)||List(Coalesce(px.descricao,''),ascii_char(13))) as xpto
   ...
   'MANEQUIM: '||ascii_char(13)||
   COALESCE(
   (SELECT LIST(distinct Coalesce(TM.descricao,'')||': '||Coalesce(PM.valor,'0')||' - '||Coalesce(TMP.descricao,'0'),ascii_char(13))
...
where Coalesce((Select min(s.sincronizado) from tabcontolesincagenda s where s.idagenda = a.codigo),A.sincronizada,0) = 0
