object frmConfiguracoes: TfrmConfiguracoes
  Left = 185
  Top = 175
  BorderIcons = []
  BorderStyle = bsSingle
  Caption = 'Configura'#231#245'es'
  ClientHeight = 527
  ClientWidth = 1019
  Color = 16312551
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poDefault
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel_btns_cad: TPanel
    Left = 939
    Top = 41
    Width = 80
    Height = 486
    Align = alRight
    BevelInner = bvLowered
    Color = 16644087
    ParentBackground = False
    TabOrder = 0
    object btn_sair: TSpeedButton
      Left = 2
      Top = 427
      Width = 76
      Height = 57
      Hint = 'Sair'
      Align = alBottom
      Caption = '&Sair'
      Flat = True
      Glyph.Data = {
        
        0000}
      Layout = blGlyphTop
      ParentShowHint = False
      ShowHint = True
      OnClick = btn_sairClick
      ExplicitLeft = 4
    end
  end
  object pnlTitulo: TPanel
    Left = 0
    Top = 0
    Width = 1019
    Height = 41
    Align = alTop
    BevelOuter = bvNone
    Caption = 'CONFIGURA'#199#195'O'
    Color = 14719638
    Font.Charset = DEFAULT_CHARSET
    Font.Color = 8924712
    Font.Height = -27
    Font.Name = 'Arial Black'
    Font.Style = []
    ParentBackground = False
    ParentFont = False
    TabOrder = 1
  end
  object Panel2: TPanel
    Left = 0
    Top = 41
    Width = 939
    Height = 486
    Align = alClient
    BevelOuter = bvNone
    Color = 16312551
    ParentBackground = False
    TabOrder = 2
    object cxGrid1: TcxGrid
      Left = 0
      Top = 0
      Width = 608
      Height = 486
      Align = alClient
      TabOrder = 0
      LookAndFeel.SkinName = 'UserSkin'
      object cxGrid1DBTableView1: TcxGridDBTableView
        NavigatorButtons.ConfirmDelete = False
        DataController.DataSource = dtsConfig
        DataController.Filter.Options = [fcoCaseInsensitive]
        DataController.Filter.Active = True
        DataController.Summary.DefaultGroupSummaryItems = <>
        DataController.Summary.FooterSummaryItems = <>
        DataController.Summary.SummaryGroups = <>
        FilterRow.Visible = True
        FilterRow.ApplyChanges = fracImmediately
        OptionsData.CancelOnExit = False
        OptionsData.Deleting = False
        OptionsData.DeletingConfirmation = False
        OptionsData.Editing = False
        OptionsData.Inserting = False
        OptionsView.GroupByBox = False
        OptionsView.Header = False
        object cxGrid1DBTableView1CONFIG_CAPTION: TcxGridDBColumn
          DataBinding.FieldName = 'CONFIG_CAPTION'
          Width = 576
        end
      end
      object cxGrid1Level1: TcxGridLevel
        GridView = cxGrid1DBTableView1
      end
    end
    object Panel3: TPanel
      Left = 608
      Top = 0
      Width = 331
      Height = 486
      Align = alRight
      BevelOuter = bvNone
      Color = 16312551
      TabOrder = 1
      object DBMemo1: TDBMemo
        Left = 0
        Top = 217
        Width = 331
        Height = 269
        Align = alClient
        BevelInner = bvNone
        BevelOuter = bvNone
        BorderStyle = bsNone
        DataField = 'DESCRICAO'
        DataSource = dtsConfig
        ParentColor = True
        ReadOnly = True
        TabOrder = 0
      end
      object Notebook1: TNotebook
        Left = 0
        Top = 81
        Width = 331
        Height = 136
        Align = alTop
        TabOrder = 1
        object TPage
          Left = 0
          Top = 0
          Caption = 'configTexto'
          object wwDBEdit1: TwwDBEdit
            Left = 6
            Top = 15
            Width = 319
            Height = 21
            Color = 16644087
            DataField = 'VALOR_STRING'
            DataSource = dtsConfig
            TabOrder = 0
            UnboundDataType = wwDefault
            WantReturns = False
            WordWrap = False
          end
        end
        object TPage
          Left = 0
          Top = 0
          Caption = 'configInteiro'
          object wwDBEdit2: TwwDBEdit
            Left = 6
            Top = 15
            Width = 121
            Height = 21
            Color = 16644087
            DataField = 'VALOR_INTEIRO'
            DataSource = dtsConfig
            TabOrder = 0
            UnboundDataType = wwDefault
            WantReturns = False
            WordWrap = False
          end
        end
        object TPage
          Left = 0
          Top = 0
          Caption = 'configMemo'
          object DBMemo2: TDBMemo
            Left = 6
            Top = 15
            Width = 319
            Height = 115
            Color = 16644087
            DataField = 'VALOR_MEMO'
            DataSource = dtsConfig
            TabOrder = 0
          end
        end
        object TPage
          Left = 0
          Top = 0
          Caption = 'configLogico'
          object DBRadioGroup1: TDBRadioGroup
            Left = 6
            Top = 15
            Width = 131
            Height = 74
            Ctl3D = False
            DataField = 'VALOR_LOGICO'
            DataSource = dtsConfig
            Items.Strings = (
              '(N'#195'O) Falso'
              '(SIM ) Verdadeiro')
            ParentBackground = True
            ParentCtl3D = False
            TabOrder = 0
            Values.Strings = (
              '0'
              '1')
          end
        end
        object TPage
          Left = 0
          Top = 0
          Caption = 'configData'
          object wwDBDateTimePicker1: TwwDBDateTimePicker
            Left = 6
            Top = 15
            Width = 139
            Height = 21
            CalendarAttributes.Font.Charset = DEFAULT_CHARSET
            CalendarAttributes.Font.Color = clWindowText
            CalendarAttributes.Font.Height = -11
            CalendarAttributes.Font.Name = 'Tahoma'
            CalendarAttributes.Font.Style = []
            Color = 16644087
            DataField = 'VALOR_DATA'
            DataSource = dtsConfig
            Epoch = 1950
            ShowButton = True
            TabOrder = 0
            UnboundDataType = wwDTEdtDate
          end
        end
        object TPage
          Left = 0
          Top = 0
          Caption = 'configHora'
          object wwDBDateTimePicker2: TwwDBDateTimePicker
            Left = 6
            Top = 15
            Width = 121
            Height = 21
            CalendarAttributes.Font.Charset = DEFAULT_CHARSET
            CalendarAttributes.Font.Color = clWindowText
            CalendarAttributes.Font.Height = -11
            CalendarAttributes.Font.Name = 'Tahoma'
            CalendarAttributes.Font.Style = []
            Color = 16644087
            DataField = 'VALOR_HORA'
            DataSource = dtsConfig
            Epoch = 1950
            ShowButton = True
            TabOrder = 0
            UnboundDataType = wwDTEdtTime
          end
        end
        object TPage
          Left = 0
          Top = 0
          Caption = 'configReal'
          object wwDBEdit3: TwwDBEdit
            Left = 6
            Top = 15
            Width = 121
            Height = 21
            Color = 16644087
            DataField = 'VALORREAL'
            DataSource = dtsConfig
            TabOrder = 0
            UnboundDataType = wwDefault
            WantReturns = False
            WordWrap = False
          end
        end
        object TPage
          Left = 0
          Top = 0
          Caption = 'configLookup'
          object lkpConfig: TRxDBLookupCombo
            Left = 6
            Top = 15
            Width = 320
            Height = 21
            DropDownCount = 8
            Color = 16644087
            DataField = 'VALOR_INTEIRO'
            DataSource = dtsConfig
            LookupSource = dtsLookup
            TabOrder = 0
          end
        end
        object TPage
          Left = 0
          Top = 0
          Caption = 'configCor'
          object Panel1: TPanel
            Left = 0
            Top = 0
            Width = 331
            Height = 136
            Align = alClient
            BevelOuter = bvNone
            Color = 16312551
            ParentBackground = False
            TabOrder = 0
            object SpeedButtonAlterarCor: TSpeedButton
              Left = 35
              Top = 96
              Width = 92
              Height = 27
              Hint = 'Escolher Cor'
              Caption = 'Alterar Cor'
              Flat = True
              Glyph.Data = {
                
                0505}
              Margin = 4
              ParentShowHint = False
              ShowHint = True
              OnClick = SpeedButtonAlterarCorClick
            end
            object ShapeCor: TShape
              Left = 6
              Top = 6
              Width = 147
              Height = 83
              Shape = stRoundSquare
            end
          end
        end
      end
      object Panel4: TPanel
        Left = 0
        Top = 0
        Width = 331
        Height = 81
        Align = alTop
        BevelOuter = bvNone
        Color = 14055796
        ParentBackground = False
        TabOrder = 2
        object DBText1: TDBText
          Left = 0
          Top = 0
          Width = 331
          Height = 81
          Align = alClient
          Alignment = taCenter
          DataField = 'CONFIG_CAPTION'
          DataSource = dtsConfig
          Font.Charset = DEFAULT_CHARSET
          Font.Color = 16312551
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = [fsBold, fsUnderline]
          ParentFont = False
          WordWrap = True
          ExplicitLeft = 32
          ExplicitTop = 16
          ExplicitWidth = 65
          ExplicitHeight = 17
        end
      end
    end
  end
  object dseConfig: TIBDataSet
    Database = Dados.IBDatabase
    Transaction = Dados.IBTransaction
    AfterPost = dseConfigAfterPost
    AfterScroll = dseConfigAfterScroll
    BeforeOpen = dseConfigBeforeOpen
    DeleteSQL.Strings = (
      'delete from TABCONFIGURACOES'
      'where'
      '  CONFIG_NAME = :OLD_CONFIG_NAME and'
      '  FORM_NAME = :OLD_FORM_NAME')
    InsertSQL.Strings = (
      'insert into TABCONFIGURACOES'
      
        '  (CONFIG_CAPTION, CONFIG_NAME, DESCRICAO, DISPLAY_LOOKUP, FORM_' +
        'NAME, KEYVALUE_LOOKUP, '
      
        '   SEQUENCIAFECHAMENTOTURNO, SQL_LOOKUP, TIPOCONFIG, VALOR_DATA,' +
        ' VALOR_HORA, '
      
        '   VALOR_INTEIRO, VALOR_LOGICO, VALOR_MEMO, VALOR_STRING, VALORR' +
        'EAL)'
      'values'
      
        '  (:CONFIG_CAPTION, :CONFIG_NAME, :DESCRICAO, :DISPLAY_LOOKUP, :' +
        'FORM_NAME, '
      
        '   :KEYVALUE_LOOKUP, :SEQUENCIAFECHAMENTOTURNO, :SQL_LOOKUP, :TI' +
        'POCONFIG, '
      
        '   :VALOR_DATA, :VALOR_HORA, :VALOR_INTEIRO, :VALOR_LOGICO, :VAL' +
        'OR_MEMO, '
      '   :VALOR_STRING, :VALORREAL)')
    RefreshSQL.Strings = (
      'Select '
      '  FORM_NAME,'
      '  CONFIG_NAME,'
      '  CONFIG_CAPTION,'
      '  DESCRICAO,'
      '  TIPOCONFIG,'
      '  VALOR_INTEIRO,'
      '  VALOR_STRING,'
      '  VALOR_MEMO,'
      '  VALOR_LOGICO,'
      '  VALOR_DATA,'
      '  VALOR_HORA,'
      '  SQL_LOOKUP,'
      '  DISPLAY_LOOKUP,'
      '  KEYVALUE_LOOKUP,'
      '  SEQUENCIAFECHAMENTOTURNO,'
      '  VALORREAL'
      'from TABCONFIGURACOES '
      'where'
      '  CONFIG_NAME = :CONFIG_NAME and'
      '  FORM_NAME = :FORM_NAME')
    SelectSQL.Strings = (
      'Select * from TABCONFIGURACOES'
      'where FORM_NAME = :F_NAME '
      'order by CONFIG_CAPTION')
    ModifySQL.Strings = (
      'update TABCONFIGURACOES'
      'set'
      '  CONFIG_CAPTION = :CONFIG_CAPTION,'
      '  CONFIG_NAME = :CONFIG_NAME,'
      '  DESCRICAO = :DESCRICAO,'
      '  DISPLAY_LOOKUP = :DISPLAY_LOOKUP,'
      '  FORM_NAME = :FORM_NAME,'
      '  KEYVALUE_LOOKUP = :KEYVALUE_LOOKUP,'
      '  SEQUENCIAFECHAMENTOTURNO = :SEQUENCIAFECHAMENTOTURNO,'
      '  SQL_LOOKUP = :SQL_LOOKUP,'
      '  TIPOCONFIG = :TIPOCONFIG,'
      '  VALOR_DATA = :VALOR_DATA,'
      '  VALOR_HORA = :VALOR_HORA,'
      '  VALOR_INTEIRO = :VALOR_INTEIRO,'
      '  VALOR_LOGICO = :VALOR_LOGICO,'
      '  VALOR_MEMO = :VALOR_MEMO,'
      '  VALOR_STRING = :VALOR_STRING,'
      '  VALORREAL = :VALORREAL'
      'where'
      '  CONFIG_NAME = :OLD_CONFIG_NAME and'
      '  FORM_NAME = :OLD_FORM_NAME')
    Left = 200
    Top = 128
    object dseConfigFORM_NAME: TIBStringField
      FieldName = 'FORM_NAME'
      Origin = '"TABCONFIGURACOES"."FORM_NAME"'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
      Size = 50
    end
    object dseConfigCONFIG_NAME: TIBStringField
      FieldName = 'CONFIG_NAME'
      Origin = '"TABCONFIGURACOES"."CONFIG_NAME"'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
      Size = 50
    end
    object dseConfigCONFIG_CAPTION: TIBStringField
      FieldName = 'CONFIG_CAPTION'
      Origin = '"TABCONFIGURACOES"."CONFIG_CAPTION"'
      Size = 100
    end
    object dseConfigDESCRICAO: TIBStringField
      FieldName = 'DESCRICAO'
      Origin = '"TABCONFIGURACOES"."DESCRICAO"'
      Size = 200
    end
    object dseConfigTIPOCONFIG: TIntegerField
      FieldName = 'TIPOCONFIG'
      Origin = '"TABCONFIGURACOES"."TIPOCONFIG"'
    end
    object dseConfigVALOR_INTEIRO: TIntegerField
      FieldName = 'VALOR_INTEIRO'
      Origin = '"TABCONFIGURACOES"."VALOR_INTEIRO"'
    end
    object dseConfigVALOR_STRING: TIBStringField
      FieldName = 'VALOR_STRING'
      Origin = '"TABCONFIGURACOES"."VALOR_STRING"'
      Size = 200
    end
    object dseConfigVALOR_MEMO: TMemoField
      FieldName = 'VALOR_MEMO'
      Origin = '"TABCONFIGURACOES"."VALOR_MEMO"'
      ProviderFlags = [pfInUpdate]
      BlobType = ftMemo
      Size = 8
    end
    object dseConfigVALOR_LOGICO: TIntegerField
      FieldName = 'VALOR_LOGICO'
      Origin = '"TABCONFIGURACOES"."VALOR_LOGICO"'
    end
    object dseConfigVALOR_DATA: TDateField
      FieldName = 'VALOR_DATA'
      Origin = '"TABCONFIGURACOES"."VALOR_DATA"'
    end
    object dseConfigVALOR_HORA: TTimeField
      FieldName = 'VALOR_HORA'
      Origin = '"TABCONFIGURACOES"."VALOR_HORA"'
    end
    object dseConfigSQL_LOOKUP: TMemoField
      FieldName = 'SQL_LOOKUP'
      Origin = '"TABCONFIGURACOES"."SQL_LOOKUP"'
      ProviderFlags = [pfInUpdate]
      BlobType = ftMemo
      Size = 8
    end
    object dseConfigDISPLAY_LOOKUP: TIBStringField
      FieldName = 'DISPLAY_LOOKUP'
      Origin = '"TABCONFIGURACOES"."DISPLAY_LOOKUP"'
      Size = 50
    end
    object dseConfigKEYVALUE_LOOKUP: TIBStringField
      FieldName = 'KEYVALUE_LOOKUP'
      Origin = '"TABCONFIGURACOES"."KEYVALUE_LOOKUP"'
      Size = 50
    end
    object dseConfigSEQUENCIAFECHAMENTOTURNO: TIntegerField
      FieldName = 'SEQUENCIAFECHAMENTOTURNO'
      Origin = '"TABCONFIGURACOES"."SEQUENCIAFECHAMENTOTURNO"'
    end
    object dseConfigVALORREAL: TFMTBCDField
      FieldName = 'VALORREAL'
      Origin = '"TABCONFIGURACOES"."VALORREAL"'
      Precision = 18
      Size = 6
    end
  end
  object dtsConfig: TDataSource
    DataSet = dseConfig
    Left = 232
    Top = 128
  end
  object qryLookup: TIBQuery
    Database = Dados.IBDatabase
    Transaction = Dados.IBTransaction
    Left = 200
    Top = 160
  end
  object dtsLookup: TDataSource
    DataSet = qryLookup
    Left = 232
    Top = 160
  end
  object ColorDialogClassificacao: TColorDialog
    Left = 203
    Top = 195
  end
end
